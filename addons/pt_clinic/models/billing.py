...
class PtTreatmentPackage(models.Model):
    ...
    company_id = fields.Many2one(
        "res.partner", string="Corporate Client", domain=[("is_company", "=", True)]
    )
    total_sessions = fields.Integer(string="Total Sessions", required=True, default=10)
    used_sessions = fields.Integer(string="Used Sessions", compute="_compute_usage", store=True)
    remaining_sessions = fields.Integer(string="Remaining Sessions", compute="_compute_usage", store=True)
    start_date = fields.Date(string="Start Date", default=fields.Date.context_today)
    end_date = fields.Date(string="End Date", index=True)
    billing_mode = fields.Selection(
        [("prepaid", "Prepaid"), ("postpaid", "Postpaid by Sessions")],
        string="Billing Mode",
        default="prepaid",
        required=True,
    )
    ...
    discount_type = fields.Selection(
        [("none", "None"), ("percent", "Percent"), ("fixed", "Fixed")], string="Discount Type", default="none"
    )
    ...
    usage_ids = fields.One2many("pt.package.usage", "package_id", string="Usage")
...
class PtPackageUsage(models.Model):
    ...
    package_id = fields.Many2one("pt.treatment.package", string="Package", required=True, ondelete="cascade")
    session_id = fields.Many2one("pt.session", string="Session", required=True, ondelete="cascade")
    clinic_company_id = fields.Many2one(
        "res.company", string="Company", related="package_id.clinic_company_id", store=True, index=True
    )
    used_on = fields.Date(string="Used On", required=True)
...
class PtDiscountOffer(models.Model):
    ...
    offer_type = fields.Selection(
        [("percent", "Percent"), ("fixed", "Fixed")], string="Offer Type", required=True, default="percent"
    )
...
class PtCompanyContract(models.Model):
    ...
    partner_id = fields.Many2one(
        "res.partner", string="Corporate Partner", required=True, domain=[("is_company", "=", True)], ondelete="restrict"
    )
    start_date = fields.Date(string="Start Date", required=True)
    end_date = fields.Date(string="End Date", required=True)
    state = fields.Selection(
        [("draft", "Draft"), ("active", "Active"), ("expired", "Expired")], string="Status", default="draft"
    )
    line_ids = fields.One2many("pt.company.contract.line", "contract_id", string="Contract Lines")
...
class PtCompanyContractLine(models.Model):
    ...
    contract_id = fields.Many2one("pt.company.contract", string="Contract", required=True, ondelete="cascade")
    clinic_company_id = fields.Many2one(
        "res.company", string="Company", related="contract_id.clinic_company_id", store=True, index=True
    )
...
class PtSessionInvoicingWizard(models.TransientModel):
    ...
    patient_id = fields.Many2one("pt.patient", string="Patient", required=True)
    session_ids = fields.Many2many("pt.session", string="Sessions", domain="[(\"patient_id\",\"=\",patient_id), (\"billed\",\"=\",False)]")
    package_id = fields.Many2one("pt.treatment.package", string="Package", domain="[(\"patient_id\",\"=\",patient_id)]")
    ...
    discount_value = fields.Float(string="Discount Value", default=0.0)
    offer_id = fields.Many2one("pt.discount.offer", string="Discount Offer", domain="[(\"active\",\"=\",True)]")
    ...
    def action_create_invoice(self):
        ...
        line_name = _("Therapy Sessions x%s - %s", qty, self.patient_id.name)
